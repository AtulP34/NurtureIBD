...
if page == "Patient Entry & Upload":
    ...
    # View collected data
    with st.expander(":mag: View Submitted Patient Info"):
...
# ------------------ PAGE 2 ------------------
    ...
    st.title(":brain: AI Analysis Assistant")
...
"""
    ...
    if not section_map:
        st.warning(":warning: Could not parse AI response properly. Showing raw output:")
        st.text(st.session_state.ai_response)
    # Show selected section
    if ai_section == "Diet Plan":
        st.subheader(":green_salad: Personalized Diet Plan")
        st.markdown(section_map.get("Diet Plan", "_No diet plan available._"))
    elif ai_section == "Medicine Schedule":
        st.subheader(":pill: Medication & Supplement Schedule")
        ...
        st.subheader(":test_tube: Blood Work Interpretation")
